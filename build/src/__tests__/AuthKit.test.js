...
const substitute_1 = require("@fluffy-spoon/substitute");
...
describe('randomStringDefault', () => {
    it('generates correct random values', () => {
        ...
        for (let i = 0; i < iterations; i++) {
            let s = (0, AuthKit_1.randomStringDefault)(32);
...
describe('AuthKit', () => {
    ...
    const makeUnit = () => {
        const $unit = new AuthKit_1.AuthKit(params(), pkceSource);
        $unit.randomString = (length) => `stub-${length}`;
        $unit.getQuery = () => query;
        $unit.redirect = (url) => (redirectTo = url);
        $unit.refreshLimit = 3;
        return $unit;
    };
    let redirectTo;
    beforeEach(async () => {
        ...
        query = '';
        error = undefined;
        pkceSource = substitute_1.Substitute.for();
        ...
        describe('tokens and userinfo already in storage', () => {
            ...
            describe('successful refresh', () => {
                ...
                it('makes call to token endpoint', async () => {
                    expect(mockAxios.post).toHaveBeenCalledWith(issuer + '/oauth/token', queryString.stringify({
                        refresh_token: refreshToken,
        ...
        describe('authentication not in storage', () => {
            describe('minimal params', () => {
                ...
                it('redirected to the endpoint', () => {
                    expect(redirectTo).toBe(`${issuer}/authorize?client_id=test-client-id&redirect_uri=${encodeURIComponent(window.location.href)}&nonce=stub-32&response_type=code&scope=scope1%20scope2&code_challenge=test-challenge`);
                });
            ...
            describe('with state', () => {
                ...
                it('redirected to the endpoint', () => {
                    expect(redirectTo).toBe(`${issuer}/authorize?client_id=test-client-id&redirect_uri=${encodeURIComponent(window.location.href)}&state=${state}&nonce=stub-32&response_type=code&scope=scope1%20scope2&code_challenge=test-challenge`);
                });
            });
            describe('with redirectUri', () => {
                ...
                it('redirected to the endpoint', () => {
                    expect(redirectTo).toBe(`${issuer}/authorize?client_id=test-client-id&redirect_uri=${encodeURIComponent(redirectUri)}&nonce=stub-32&response_type=code&scope=scope1%20scope2&code_challenge=test-challenge`);
                });
            });
            describe('with post binding', () => {
                let form;
                beforeEach(() => {
                    unit.submitForm = ($form) => {
                        form = $form;
                    };
                    pkceSource.create().returns({
                        challenge,
